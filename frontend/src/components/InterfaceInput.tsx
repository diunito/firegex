import { Combobox, ComboboxProps, InputBase, TextInput, TextInputProps, useCombobox } from "@mantine/core";
...
export const InterfaceInput = ({ initialCustomInterfaces, includeInterfaceNames, onChange, value, defaultValue }:InterfaceInputProps) => {
    ...
    const exactOptionMatch = data.find((item) => item.value === search);

    const filteredOptions = exactOptionMatch?[exactOptionMatch]:data.filter((item) => item.value.toLowerCase().includes(search.toLowerCase().trim()));
